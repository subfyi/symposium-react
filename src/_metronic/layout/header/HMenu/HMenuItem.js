...
const user = memoizeOne(() => tokenized.get("/api/myself"));
...
export default class HMenuItem extends React.Component {
  ...
  checkMenuPermission(menu) {
    if(menu.takipcisi && menu.takipcisi.indexOf(Number(this.state.user.takipcisi)) === -1) {
      return false;
    }

    if(menu.yetki && menu.yetki < this.state.user.yetki) {
      return false;
    }
  ...
  render() {
    ...
    if(!this.state.user || !this.checkMenuPermission(item)) 
      return null;
          ...
          >
            ...
            {item.submenu && item.submenu.length && (
              <ul className="kt-menu__subnav">
                {item.submenu.filter(menu => this.checkMenuPermission(menu)).map((child, index) => {
                  return (
            ...
            {item.submenu.items && item.submenu.items.length && (
              <ul className="kt-menu__subnav">
                {item.submenu.items.filter(menu => this.checkMenuPermission(menu)).map((child, index) => {
                  return (
                    ...
                    {item.submenu.columns.map((child, index) => {
                      return (
                        <React.Fragment key={`hmenuColumnInner${index}`}>
                          <HMenuColumn
                            item={{ items: child.items.filter(menu => this.checkMenuPermission(menu)) }}
                            parentItem={item}
                            currentUrl={currentUrl}
                          />
                        </React.Fragment>
