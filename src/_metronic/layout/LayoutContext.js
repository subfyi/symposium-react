...
function findPageConfig(currentPage, items, breadcrumbs) {
  ...
  if (!items || (!Array.isArray(items) && items.type !== "mega")) {
    return;
  }

  if(items.type == "mega") {
    items = items.columns.map(a => a.items).reduce((a, b) => a.concat(b), []);
  }
...
function init({ pathname, menuConfig }) {
  ...
  let breadcrumbs = [];
  const pageConfig =
      findPageConfig(currentPage, menuConfig.aside && menuConfig.aside.items, breadcrumbs) ||
      findPageConfig(currentPage, menuConfig.header && menuConfig.header.items, breadcrumbs);
