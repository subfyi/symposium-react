...
export default class TakipciSelect extends Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    if(this.props.value) {
      ...
      if(this.props.cachedValue && this.props.value == this.props.cachedValue.id) {
        ...
        data = (await tokenized.get(`/api/kullanicilar/${this.props.value}`)).data;
      }

      this.setState({ selected: ({ value: data.id, label: data.takipcisi }) });
    }
  }
  ...
  async loadData(arama) {
    ...
    var data = await tokenized.get(`/api/kullanicilar`, { params: { query: arama }, cancelToken: this.cancelToken.token });
    return data.data.data
      .filter(data => data.id !== 17)
      .map(data => ({ value: data.id, label: data.takipcisi }));
  }
